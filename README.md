...
2.) Install [leat-client](https://github.com/ileathan/leat-client). (edit the end of the `leat-mine.js` file with your server information and then host (or use javascript to set them up [see the readme's]). `leathash.wasm`, `leathash-asmjs.min.js`, `leathash-asmjs.min.js.mem` and `leat-mine.js` on your server then. **The `miner.html` file is not needed (its na embedable miner) see [embed-miner](https://github.com/ileathan/embed-miner).**
